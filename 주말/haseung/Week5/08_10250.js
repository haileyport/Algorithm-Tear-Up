...
for (let i = 0; i < first; i++) {
  ...
  const H = Number(arr[0]); // 호텔 층수,
  const N = Number(arr[2]); //몇번째 손님

  let topFloor, roomNumber; //층 수와 호수

  //손님이 있는 층이 꼭대기 층이라면
  topFloor = N % H === 0 ? H : N % H;
  roomNumber = N % H === 0 ? N / H : Math.ceil(N / H);

  if (roomNumber < 10) {
    //호수가 10보다 작으면
    roomNumber = `0${roomNumber}`; //0을 붙여주고,
  }
  console.log(`${topFloor}${roomNumber}`); //10보다 크면 그대로 출력
}
